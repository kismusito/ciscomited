                    ...
                    <div className="center_container">
                        <div className="container_white_edit">
                            <div className="header_edit_profile">
                                <h4>Editar perfil</h4>
                                <p className="profile_password_change">Recuerda que si queres cambiar tu contraseña, debes ir a la pestaña de configuración.</p>
                            </div>
                            <form method="POST" encType="multipart/form-data" onSubmit={this.editProfileTriggerSubmit}>
                                <div className="form_group center_horizontal">
                                    <img src={authReducer.userInfo.profilePicture ? authReducer.userInfo.profilePicture : "/assets/img/usuario.png"} ref={input => this.profilePicturePreview = input} className="profile_picture_change" alt="edit custom avatar" />
                                    <div className="cameraOverlayTop">
                                        <div className="cameraOverlay">
                                            camera
                                            <input type="file" name="imageProfile" ref={input => this.profilePicture = input} onChange={this.changeImagePreview} className="profile_picture_select" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="form_group">
                                    <div className="rows">
                                        <div className="col_6">
                                            ...
                                            {!editProfileReducer.status && editProfileReducer.error === "user" &&
                                                <div className="alert_error_edit">{editProfileReducer.message}</div>
                                            }
                                        </div>
                                        <div className="col_6">
                                            <input
                                            ...
                                            {!editProfileReducer.status && editProfileReducer.error === "email" &&
                                                <div className="alert_error_edit">{editProfileReducer.message}</div>
                                            }
                                        </div>
                                    </div>
                                ...
                                <div className="links_section_form">
                                    <button className="btn btn_big btn_orange">Editar perfil</button>
                                </div>

                                {!editProfileReducer.status && editProfileReducer.error === "update" &&
                                    <div className="alert_error_edit">{editProfileReducer.message}</div>
                                }

                                {editProfileReducer.status && editProfileReducer.error === "update" &&
                                    <div className="alert_success_edit">{editProfileReducer.message}</div>
                                }
