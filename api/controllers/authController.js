...
authMethods.login = async (req, res) => {
    ...
    if (validateUserOrEmail) {
        ...
        if (validate) {
            const token = jwt.sign(validateUserOrEmail._id.toString(), process.env.SECRECT_KEY_TOKEN)
            if (token) {
                const userInfo = await User.findOne({_id: validateUserOrEmail._id} , {username: true , email: true , user_role: true , first_name: true , last_name: true , profilePicture: true})
                setTimeout( _ => {
                    return res.json({
                        auth: true,
                        token: token,
...
authMethods.validateToken = async (req , res) => {
    ...
    if (token) {
        ...
        if(validate) {
            ...
            const userInfo = await User.findOne({_id: validate} , {username: true , email: true , user_role: true , first_name: true , last_name: true , profilePicture: true})
            return res.json({
                auth: true,
                token: token,
                userInfo: userInfo,
