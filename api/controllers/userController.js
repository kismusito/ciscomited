...
userMethods.searchUsers = async (req, res) => {
    ...
    switch (type) {
        case "user":
            const searchUsername = await User.find({ username: { $regex: searchValue, $options: 'i' } }, { username: true, email: true, profilePicture: true })
            return res.json({
                status: true,
        ...
        case "email":
            const searchEmail = await User.find({ email: { $regex: searchValue, $options: 'i' } }, { username: true, email: true, profilePicture: true })
            return res.json({
                status: true,
                users: searchEmail,
                message: "Se encontraron usuarios"
...
userMethods.searchUser = async (req, res) => {
    ...
    if (userSearchID) {
        const getUser = await User.findOne({ _id: userSearchID }, { username: true, email: true, first_name: true, last_name: true })
